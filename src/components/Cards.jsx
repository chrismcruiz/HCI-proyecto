...
function Cards({ userData, token, idUser }) {
  ...
  const revisarLikes = (idUser, idPersonLiked) => {
    let match = false;
    const liked = recorrerObjeto(
      db.filter((person) => person._id === idPersonLiked)
    ).liked;
    ...
    <div className="card__container position-relative">
      <div className="div_contenedor_personas">
        {users.map((user, index) => (
          ...
          >
            <div className="card p-0" ref={card_personas}>
              <div
              ...
              >
                <div
                  className="position-absolute div_info_personas text-tarjetas"
                  onClick={mostrarInfo}
