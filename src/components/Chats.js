...
const Chats = ({ userData, socket, usersData }) => {
  ...
  useEffect(async () => {
    try {
      if (idTarjeta) {
        const response = await axios.post(
          "http://localhost:4000/app/createConversation",
          [userData._id, idTarjeta]
        );
    ...
    try {
      const req = await axios.get(
        "http://localhost:4000/app/getConversations?_id=" + userData._id
      );
      ...
      const idsConvers = [];
      let ultimosMensajes = [];
      const counterPartUsers = conversaciones.map((conversation) =>
        conversation.participants
          .filter((participant) => participant !== userData._id)
          .toString()
      );
      ...
      try {
        ultimosMensajes = await axios.get(
          "http://localhost:4000/app/getLastMessages"
        );
      ...
      const usuarios = usersData.filter((user) =>
        counterPartUsers.includes(user._id)
      );
      ...
      conversaciones.map((conversation) => {
        dataConvers.map((obj) => {
          ...
          if (obj.room === conversation._id) {
            const idUser = conversation.participants.filter(
              (participant) => participant !== userData._id
            )[0];
            // console.log(idUser)
            const usuarioData = usuarios.filter(
  ...
  const handleInputChange = (e) => {
    ...
    if (str !== "") {
      let debug = search.filter((chat) =>
        chat["name"].toLowerCase().includes(str)
      );
      setChats(debug);
    } else {
      setChats(search);
        ...
        <div className="row">
          ...
          <div className="col-8 position-relative border-start">
            <ChatScreen
              ...
              actualizarMensajes={actualizarMensajesIzquierda}
              enviado={enviado}
